...
function playbackProviderLabel(song) {
  ...
  if (provider === 'kw' || provider === 'kuwo') return '酷我音乐';
  if (provider === 'mg' || provider === 'migu') return '咪咕音乐';
  if (provider === 'free') return '免费音源';
  return '网易云';
}
...
function isSameTitleArtist(source, candidate) {
  ...
  // 衍生版本防御：候选是衍生版（Live/Remix/翻唱/伴奏等）但原曲不是 → 拒绝，避免歌对版不对
  if (typeof searchLooksLikeDerivative === 'function') {
    var _srcRaw = String(((source && source.name) || '') + ' ' + ((source && source.artist) || '') + ' ' + ((source && source.album) || '')).toLowerCase();
    var _candRaw = String(((candidate && candidate.name) || '') + ' ' + ((candidate && candidate.artist) || '') + ' ' + ((candidate && candidate.album) || '')).toLowerCase();
    if (searchLooksLikeDerivative(_candRaw) && !searchLooksLikeDerivative(_srcRaw)) return false;
  }
...
var SOURCE_FALLBACK_DIRECT_PROVIDERS = ['netease', 'qq', 'kugou', 'kw', 'mg', 'free'];
...
var FREE_SOURCE_CACHE_KEY = 'mr_free_source_cache_v3';
function freeSourceCacheKey(song) {
  var provider = songProviderKey(song) || '';
  return [provider, song.id || song.mid || '', (song.name || song.title || '').trim(), (song.artist || '').trim()].join('::');
}
function loadFreeSourceCache() {
  try { var raw = localStorage.getItem(FREE_SOURCE_CACHE_KEY); return raw ? JSON.parse(raw) : {}; } catch (e) { return {}; }
}
function saveFreeSourceCache(cache) {
  try { localStorage.setItem(FREE_SOURCE_CACHE_KEY, JSON.stringify(cache)); } catch (e) {}
}
function getCachedFreeSong(song) {
  var cache = loadFreeSourceCache();
  var key = freeSourceCacheKey(song);
  var entry = cache[key];
  if (!entry || !entry.song) { return null; }
  if (Date.now() - (entry.ts || 0) > 7 * 24 * 3600 * 1000) { delete cache[key]; saveFreeSourceCache(cache); return null; }
  return { song: entry.song, key: key };
}
function setCachedFreeSong(originalSong, freeSong) {
  var cache = loadFreeSourceCache();
  var key = freeSourceCacheKey(originalSong);
  if (!freeSong.__lyricId) freeSong.__lyricId = originalSong.id || originalSong.mid || originalSong.songmid || '';
  if (!freeSong.__lyricProvider) freeSong.__lyricProvider = typeof songProviderKey === 'function' ? songProviderKey(originalSong) : (originalSong.provider || 'netease');
  cache[key] = { song: freeSong, ts: Date.now() };
  var keys = Object.keys(cache);
  if (keys.length > 200) {
    keys.sort(function (a, b) { return (cache[b].ts || 0) - (cache[a].ts || 0); });
    var trimmed = {};
    for (var i = 0; i < 200; i++) { trimmed[keys[i]] = cache[keys[i]]; }
    cache = trimmed;
  }
  saveFreeSourceCache(cache);
}
function clearCachedFreeSong(song) {
  var cache = loadFreeSourceCache();
  delete cache[freeSourceCacheKey(song)];
  saveFreeSourceCache(cache);
}
async function searchAlternatePlatformSong(song, requestedTarget, recovery) {
  ...
  if (target === 'free') {
    // 优先命中后台预扫描建立的 ID 映射（跳过现场搜同名曲，加速无感换源）
    if (typeof getMappedFreeSong === 'function') {
      var mapped = getMappedFreeSong(song);
      if (mapped) {
        var mappedSong = cloneSong(mapped);
        mappedSong.__fromFreeSourceCache = true;
        setCachedFreeSong(song, mappedSong);
        return mappedSong;
      }
    }
    var cached = getCachedFreeSong(song);
    if (cached) {
      var cachedSong = cloneSong(cached.song);
      cachedSong.__fromFreeSourceCache = true;
      // 防御性补填：旧缓存可能未存储 __lyricId/__lyricProvider
      if (!cachedSong.__lyricId) cachedSong.__lyricId = song.id || song.mid || song.songmid || '';
      if (!cachedSong.__lyricProvider) cachedSong.__lyricProvider = typeof songProviderKey === 'function' ? songProviderKey(song) : (song.provider || 'netease');
      return cachedSong;
    }
    var freeArtist = artistNameParts(song)[0] || '';
    var freeQuery = [song.name || song.title || '', song.artist || freeArtist].filter(Boolean).join(' ').trim();
    if (!freeQuery) return null;
    try { fetch('/api/debug-log', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({msg:'[FREE-SEARCH] query='+freeQuery+' origName='+(song.name||song.title||'')+' origArtist='+(song.artist||freeArtist||'')}) }); } catch(e){}
    var freePlatforms = ['kw', 'mg', 'lx'];
    for (var pi = 0; pi < freePlatforms.length; pi++) {
      var freePlat = freePlatforms[pi];
      var freeSearchUrl = freePlat === 'kw'
        ? '/api/free/search/kuwo?keyword=' + encodeURIComponent(freeQuery) + '&limit=8'
        : freePlat === 'mg'
        ? '/api/free/search/migu?keyword=' + encodeURIComponent(freeQuery) + '&limit=8'
        : '/api/free/search/lx?keyword=' + encodeURIComponent(freeQuery) + '&limit=8';
      if (recovery && !sourceFallbackRecoveryCanContinue(recovery)) return null;
      var freeSearchData = await awaitSourceFallbackBudget(
        apiJson(freeSearchUrl, { timeoutMs: SOURCE_FALLBACK_SEARCH_TIMEOUT_MS }),
        recovery
      );
      if (freeSearchData === sourceFallbackBudgetTimeoutResult || (recovery && !sourceFallbackRecoveryCanContinue(recovery))) return null;
      var freeList = freeSearchData && (freeSearchData.list || freeSearchData.songs || freeSearchData.result || []);
      if (!Array.isArray(freeList)) continue;
      for (var fi = 0; fi < freeList.length; fi++) {
        if (typeof sourceCandidateRejectReason === 'function' && sourceCandidateRejectReason(song, freeList[fi], freePlat)) continue;
        // 二次防御：直接检查候选歌曲原始名称是否含衍生版本标识
        var _candRaw = String(((freeList[fi] && freeList[fi].name) || '') + ' ' + ((freeList[fi] && freeList[fi].artist) || '') + ' ' + ((freeList[fi] && freeList[fi].album) || '')).toLowerCase();
        if (typeof searchLooksLikeDerivative === 'function' && searchLooksLikeDerivative(_candRaw)) { try { fetch('/api/debug-log', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({msg:'[FREE-MATCH] SKIP derivative plat='+freePlat+' cand='+((freeList[fi]&&freeList[fi].name)||'')+' - '+((freeList[fi]&&freeList[fi].artist)||'')}) }); } catch(e){} continue; }
        if (isSameTitleArtist(song, freeList[fi])) {
          try { fetch('/api/debug-log', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({msg:'[FREE-MATCH] MATCHED plat='+freePlat+' cand='+((freeList[fi]&&freeList[fi].name)||'')+' - '+((freeList[fi]&&freeList[fi].artist)||'')+' candId='+(freeList[fi].id||freeList[fi].mid||'')}) }); } catch(e){}
          var matchedFreeSong = cloneSong(freeList[fi]);
          matchedFreeSong.provider = 'free';
          matchedFreeSong.__freePlatform = freePlat;
          if (typeof songProviderKey === 'function') matchedFreeSong.__providerKey = 'free';
          if (!matchedFreeSong.playbackProvider) matchedFreeSong.playbackProvider = 'free';
          setCachedFreeSong(song, matchedFreeSong);
          return matchedFreeSong;
        }
      }
    }
    return null;
...
function prefetchFreeSourceForSong(song) {
  if (!song || song.type === 'local' || song.type === 'podcast') return;
  var provider = songProviderKey(song);
  if (provider !== 'netease' && provider !== 'qq' && provider !== 'kugou') return;
  if (getCachedFreeSong(song)) return;
  var query = [(song.name || song.title || ''), (song.artist || '')].filter(Boolean).join(' ').trim();
  if (!query) return;
  var platforms = ['kw', 'mg', 'lx'];
  function tryPlatform(pi) {
    if (pi >= platforms.length) return;
    var plat = platforms[pi];
    var url = plat === 'kw'
      ? '/api/free/search/kuwo?keyword=' + encodeURIComponent(query) + '&limit=3'
      : plat === 'mg'
      ? '/api/free/search/migu?keyword=' + encodeURIComponent(query) + '&limit=3'
      : '/api/free/search/lx?keyword=' + encodeURIComponent(query) + '&limit=3';
    try {
      fetch(url).then(function (r) { return r.json(); }).then(function (data) {
        var list = data && (data.list || data.songs || data.result || []);
        if (!Array.isArray(list)) { tryPlatform(pi + 1); return; }
        for (var j = 0; j < list.length; j++) {
          if (typeof sourceCandidateRejectReason === 'function' && sourceCandidateRejectReason(song, list[j], plat)) continue;
          var _candRaw = String(((list[j] && list[j].name) || '') + ' ' + ((list[j] && list[j].artist) || '') + ' ' + ((list[j] && list[j].album) || '')).toLowerCase();
          if (typeof searchLooksLikeDerivative === 'function' && searchLooksLikeDerivative(_candRaw)) continue;
          if (isSameTitleArtist(song, list[j])) {
            var matched = cloneSong(list[j]);
            matched.provider = 'free';
            matched.__freePlatform = plat;
            if (typeof songProviderKey === 'function') matched.__providerKey = 'free';
            if (!matched.playbackProvider) matched.playbackProvider = 'free';
            setCachedFreeSong(song, matched);
            return;
          }
        }
        tryPlatform(pi + 1);
      }).catch(function () { tryPlatform(pi + 1); });
    } catch (e) { tryPlatform(pi + 1); }
  }
  tryPlatform(0);
}
function prefetchQueueFreeSources(fromIdx, count) {
  if (typeof playQueue === 'undefined' || !Array.isArray(playQueue)) return;
  count = count || 5;
  var endIdx = Math.min(fromIdx + count, playQueue.length);
  for (var i = fromIdx + 1; i < endIdx; i++) {
    prefetchFreeSourceForSong(playQueue[i]);
  }
}
async function tryAutoPlaybackFallback(song, data, idx, token, opts) {
  var _flog = function(m) { try { fetch('/api/debug-log', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ msg: m }) }); } catch(e) {} };
  opts = opts || {};
  _flog('[FALLBACK] ENTER song=' + (song && (song.name || song.title)) + ' data.trial=' + (data && data.trial) + ' data.reason=' + (data && data.reason));
  if (opts.fallbackDepth > 0) {
    _flog('[FALLBACK] fallbackDepth>0, restoring and returning false');
  ...
  _flog('[FALLBACK] category=' + category + ' alternateProviders=' + JSON.stringify(alternateProviders));
  ...
  _flog('[FALLBACK] recovery=' + !!recovery);
  if (!recovery) return false;
  ...
  for (var providerIndex = 0; providerIndex < alternateProviders.length; providerIndex++) {
    ...
    try {
      _flog('[FALLBACK] searching alternate on ' + alternateProvider);
      var alternate = await searchAlternatePlatformSong(song, alternateProvider, recovery);
      _flog('[FALLBACK] searchAlternate result for ' + alternateProvider + ' = ' + !!alternate);
      if (token !== trackSwitchToken) return false;
      if (!sourceFallbackRecoveryCanContinue(recovery)) {
        return settleSourceFallbackTerminal(idx, token, '自动恢复已达到时间上限，请稍后手动重试。', skipOpts);
      }
      ...
      var alternateData = typeof resolveAlbumGaplessPlaybackData === 'function'
        ...
        : null;
      _flog('[FALLBACK] alternateData url=' + (alternateData && alternateData.url ? 'GOT URL' : (alternateData ? 'NO URL' : 'NULL')));
      if (token !== trackSwitchToken) return false;
      if (alternateData === sourceFallbackBudgetTimeoutResult || !sourceFallbackRecoveryCanContinue(recovery)) {
        return settleSourceFallbackTerminal(idx, token, '自动恢复已达到时间上限，请稍后手动重试。', skipOpts);
      }
      if (!alternateData || !alternateData.url) { _flog('[FALLBACK] skip - no url'); continue; }
      ...
      if (fallbackStarted === true) {
        ...
        setTimeout(function () { prefetchQueueFreeSources(idx, 5); }, 0);
      ...
      if (committedCandidate && committedCandidate.__fromFreeSourceCache && alternateProvider === 'free') {
        clearCachedFreeSong(originalSong);
        var attemptKey = sourceFallbackProviderAttemptKey(recovery, song, alternateProvider);
        delete recovery.attemptedProviderKeys[attemptKey];
        if (recovery.providerAttempts > 0) recovery.providerAttempts--;
        providerIndex--;
        continue;
      }
      token = fallbackToken;
