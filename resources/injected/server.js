...
const {
  ...
  getServerStatus,
  searchFreeSource,
  reloadConfig: reloadFreeSourceConfig,
  getSongIdForPlatform,
  PLATFORM_MAP,
} = require('./free-source-api');
const {
  search: lxSearch,
  getMusicUrl: lxGetMusicUrl,
  getStatus: lxGetStatus,
  init: lxInit,
} = require('./lx-runner');
...
const server = http.createServer(async (req, res) => {
  ...
  // ---------- 免费音源聚合 URL ----------
  if (pn === '/api/free/song/url') {
    try {
      const platform = url.searchParams.get('platform') || '';
      const songId = url.searchParams.get('id') || url.searchParams.get('mid') || url.searchParams.get('hash') || '';
      const quality = url.searchParams.get('quality') || '320k';
      const songName = url.searchParams.get('name') || '';
      const songArtist = url.searchParams.get('artist') || '';
      const fallbackStr = url.searchParams.get('fallbackPlatforms') || 'tx,wy,kg,kw,mg';
      const fallbackPlatforms = fallbackStr.split(',').map(s => s.trim()).filter(Boolean);

      if (!platform && !songId) {
        sendJSON(res, { error: 'Missing platform and id' }, 400);
        return;
      }
      ...
      if (platform === 'lx') {
        ...
        result = await lxGetMusicUrl(lxSongInfo, quality);
        if (result && result.url) {
          result.source = 'lx-runner';
        }
      } else if (songId && PLATFORM_MAP[platform]) {
        result = await resolveFreeUrl(platform, songId, quality, songName, songArtist);
      } else if (songId) {
        // Try with fallback platforms
        result = await resolveFreeUrlMultiPlatform(
          { id: songId, songmid: songId, hash: songId },
          fallbackPlatforms,
          quality
        );
      }
      ...
      if (result && result.url) {
        ...
        sendJSON(res, { error: 'No free URL available' }, 404);
      }
    } catch (err) {
      console.error('[FreeSongUrl]', err);
      sendJSON(res, { error: err.message || 'free source failed' }, 500);
    }
    return;
  }

  // ---------- 免费音源维护状态 ----------
  if (pn === '/api/free/status') {
    try {
      sendJSON(res, getServerStatus());
    } catch (err) {
      sendJSON(res, { error: err.message }, 500);
    }
    return;
  ...
  // ---------- 免费音源配置重载 ----------
  if (pn === '/api/free/reload') {
    try {
      const result = reloadFreeSourceConfig();
  ...
  // ---------- 酷我音乐直接搜索 ----------
  if (pn === '/api/free/search/kuwo') {
    try {
      const keyword = url.searchParams.get('keyword') || '';
      const page = parseInt(url.searchParams.get('page')) || 1;
      const limit = parseInt(url.searchParams.get('limit')) || 30;
      const result = await searchFreeSource('kw', keyword, page, limit);
      sendJSON(res, result);
    } catch (err) {
      sendJSON(res, { error: err.message }, 500);
    }
  ...
  // ---------- 咪咕音乐直接搜索 ----------
  if (pn === '/api/free/search/migu') {
    try {
      const keyword = url.searchParams.get('keyword') || '';
      const page = parseInt(url.searchParams.get('page')) || 1;
      const limit = parseInt(url.searchParams.get('limit')) || 30;
      const result = await searchFreeSource('mg', keyword, page, limit);
      sendJSON(res, result);
    } catch (err) {
      sendJSON(res, { error: err.message }, 500);
    }
  ...
  // ---------- LX 音源搜索（聚合 6 大音源）----------
  if (pn === '/api/free/search/lx') {
    try {
      const keyword = url.searchParams.get('keyword') || '';
      const page = parseInt(url.searchParams.get('page')) || 1;
      const limit = parseInt(url.searchParams.get('limit')) || 30;
      const result = await lxSearch(keyword, page, 'music');
      result.limit = limit;
      if (result.list && result.list.length > limit) {
        result.list = result.list.slice(0, limit);
      }
      sendJSON(res, result);
    } catch (err) {
      sendJSON(res, { error: err.message }, 500);
  ...
  if (pn === '/api/free/song/lxurl') {
    try {
      ...
      const result = await lxGetMusicUrl(songInfo, quality);
      if (result && result.url) {
        sendJSON(res, {
          url: result.url,
          source: 'lx-runner',
        ...
        sendJSON(res, { error: 'No LX URL available' }, 404);
      }
    } catch (err) {
      sendJSON(res, { error: err.message }, 500);
    }
  ...
  // ---------- LX 音源运行器状态 ----------
  if (pn === '/api/free/lx/status') {
    try {
      sendJSON(res, lxGetStatus());
    } catch (err) {
      sendJSON(res, { error: err.message }, 500);
    }
    return;
